...
def _make_K(lam, eps=0):
    ...
    K = (s_expanded - s_expanded_t) / ((s_expanded - s_expanded_t) * (s_expanded - s_expanded_t) + eps**2)
...
def _eigh_fwd(A, eps):
    V, w = symmetric_eigh(A, eps)
    return (V, w), (V, w)


def _eigh_bwd(eps, res, g):
    V, w = res
    g_V, g_w = g
...
symmetric_eigh.defvjp(_eigh_fwd, _eigh_bwd)
